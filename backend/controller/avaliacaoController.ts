...
export const create = async (req: Request, res: Response) => {
  const usuarioId = (req as any).userId; // opcional
  const { tipo, referenciaId, nota, comentario } = req.body;

  const resultado = avaliacaoService.criarAvaliacao({
      tipo,
      referenciaId,
      nota,
      comentario,
...
export const listarPorReferencia = async (req: Request, res: Response) => {
  ...
  const avaliacoes = avaliacaoService.listarAvaliacoesPorReferencia(
      tipo as any,
      id,
      limit,
      page
  );
  const estat = avaliacaoService.obterEstatisticas(tipo as any, id);
...
export default { create, listarPorReferencia, getById, update, remove };